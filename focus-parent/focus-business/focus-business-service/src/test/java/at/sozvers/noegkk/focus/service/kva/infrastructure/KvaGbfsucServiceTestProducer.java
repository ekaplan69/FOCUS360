...
public class KvaGbfsucServiceTestProducer {
  ...
  private static final Logger log                   = LoggerFactory.getLogger(KvaGbfsucServiceTestProducer.class);

  @Produces
  @KVAGbfsucService
  @ApplicationScoped
  public KvaGbfsucClient init() throws IOException {
	...
	if(user == null || pass == null || url == null || systemmodus == null || verarbeitungsmdous == null) {
	  log.error("Could not read parameter from properties");
	  throw new RuntimeException("Could not read parameter from properties");
	}

	return new KvaGbfsucClient(url, verarbeitungsmdous, systemmodus, user, pass, quellsystemid, berechtigungssystemid, primaersystemid);
  }
}
