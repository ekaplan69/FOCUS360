...
@Dependent
public class DtoPartnerListeLEKIFactory {
  ...
  public DtoPartnerListeLEKI createDtoPartnerListeLEKIWithFachschluessel(Fachschluessel fachschluessel) {
	DtoPartnerLEKI input = getDtoPartnerLEKI(fachschluessel, null);

	DtoPartnerListeLEKI inputList = new DtoPartnerListeLEKI();

	DtoRequestHeader dtoRequestHeader = getDtoRequestHeader();

	inputList.setDtoRequestHeader(dtoRequestHeader);

	inputList.getDtoPartnerLEKIListe().add(input);

	return inputList;
  }

  private static DtoPartnerLEKI getDtoPartnerLEKI(Fachschluessel fachschluessel, String partnerrollenartKurzAnforderung) {
	DtoPartnerLEKI                   input                         = new DtoPartnerLEKI();
  ...
  public DtoPartnerListeLEKI createDtoPartnerListeLEKIWithPartnerId(String partnerId) {
	Fachschluessel fachschluessel = new Fachschluessel();
	fachschluessel.setFachschluessel(partnerId);
	fachschluessel.setFachschluesselArt("PARID");

	DtoPartnerLEKI input1 = getDtoPartnerLEKI(fachschluessel, "VS");
	DtoPartnerLEKI input2 = getDtoPartnerLEKI(fachschluessel, "DG");
	DtoPartnerLEKI input3 = getDtoPartnerLEKI(fachschluessel, "LE");
	...
	inputList.getDtoPartnerLEKIListe().add(input1);
	inputList.getDtoPartnerLEKIListe().add(input2);
	inputList.getDtoPartnerLEKIListe().add(input3);
